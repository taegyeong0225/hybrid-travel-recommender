...
async def lifespan(app: FastAPI):
    utils_path = os.path.join(BASE_DIR, "utills", "link_images_with_caption.py")
    parquet_path = os.path.join(BASE_DIR, "ml", "tn_visit_area_info_with_images.parquet")

    if not os.path.exists(parquet_path):
        logging.info(f"Running preprocessing script: {utils_path}")
        subprocess.run(["python", utils_path], capture_output=True, text=True)
    yield  # 앱 실행
    # 종료 시 cleanup 가능 (여기선 pass)
