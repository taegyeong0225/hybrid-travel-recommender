...
# 관광지 추천 시스템
관광객 방문 데이터를 기반으로, 사용자별로 선호할 만한 관광지 상위 5곳을 추천하는 협업 필터링 모델

# 주요 피처
userID (TRAVELER_ID): 여행자 고유 식별자
itemID (VISIT_AREA_NM): 방문 관광지명
rating: 사용자의 관광지 만족도 점수
    3개 지표의 평균값으로 계산: (만족도 + 재방문의향 + 추천의향) / 3
SIDO: 관광지가 속한 시도(광역자치단체)

# 데이터 필터링 조건
VISIT_AREA_TYPE_CD: 1~8번 유형의 관광지만 선택 (특정 카테고리의 관광지)
2회 이상 방문 기록이 있는 관광지만 포함 (groupby filter)

## 데이터 전처리
주소 기반 관광지명 통일 : 같은 주소(LOTNO_ADDR)에서 가장 빈번한 VISIT_AREA_NM으로 통일
                   : 표기가 다른 같은 장소를 하나로 통합
Rating 계산 : ratings = (DGSTFN + REVISIT_INTENTION + RCMDTN_INTENTION) / 3

# 1~5점 척도
모델 선택: SVD (Singular Value Decomposition) - Surprise 라이브러리의 SVD 알고리즘 사용

행렬 분해(Matrix Factorization) 기반 협업 필터링
User-Item 평점 행렬을 저차원 잠재 요인(latent factors)으로 분해

3.3 하이퍼파라미터 튜닝
GridSearchCV를 통한 최적 파라미터 탐색
pythonparam_grid = {
    'n_factors': [50, 100, 200],      # 잠재 요인 개수
    'n_epochs': [10, 50],              # 학습 반복 횟수
    'lr_all': [0.01, 0.1],             # 학습률
    'reg_all': [0.01, 0.1],            # 정규화 계수
    'reg_bu': [0.01, 0.1],             # User bias 정규화
    'reg_bi': [0.01, 0.1]              # Item bias 정규화
}

5-Fold 교차검증으로 RMSE, MAE 기준 최적 모델 선정
3.4 최종 모델 학습
선정된 최적 파라미터
SVD(n_factors=50, lr_all=0.01, reg_all=0.1, 
    n_epochs=50, reg_bu=0.1, reg_bi=0.1)
...
모든 User-Item 조합에 대해 예측 평점 생성

Step 2: 지역 기반 필터링
# 사용자가 선호하는 주요 SIDO(시도) 식별
# 해당 지역 내 관광지만 필터링
Step 3: 방문 이력 제외

이미 방문한 관광지는 추천 목록에서 제거

Step 4: Top-5 추천

예측 평점 기준 상위 5개 관광지 선정
사용자의 주요 선호 지역 내에서만 추천
---------------------------
...
visit_E = pd.read_csv(os.path.join(DATA_DIR, "tn_visit_area_info_E.csv"))
id_list=['E']
...
df1.to_csv(os.path.join(PREPROC_DIR, "dfE.csv"))
...
file_path = os.path.join(SAVEMODEL_DIR, "model/svd_model_E.pkl")
# Save the model to the specified .pkl file
with open(file_path, 'wb') as file:
    pickle.dump(algo, file)

loaded_model1 = joblib.load(os.path.join(SAVEMODEL_DIR, 'model/svd_model_E.pkl'))
...
df_E_final.to_csv(os.path.join(SAVEMODEL_DIR, 'result/testset_output/E_test_ouput.csv'))
...
filtered_top_recommendations.to_csv(os.path.join(SAVEMODEL_DIR, 'result/final_output/E_top_recommendations.csv'))
